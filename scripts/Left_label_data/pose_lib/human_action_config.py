...
config.SDKParamType.HYAI_MOBILE_SDK_PARAM_FACEACTION_EYE_THRESHOLD = (
    2  # 设置左闭眼或者右闭眼的阈值 范围[0， 100]，值越小越灵敏， 默认值50
)
config.SDKParamType.HYAI_MOBILE_SDK_PARAM_FACEACTION_MOUTH_THRESHOLD = (
    3  # 设置嘴巴张开的阈值 范围[0， 100]，值越小越灵敏， 默认值50
)
config.SDKParamType.HYAI_MOBILE_SDK_PARAM_FACEACTION_HEAD_THRESHOLD = (
    4  # 设置左右摇头的阈值 范围[0， 100]，值越小越灵敏， 默认值50
)
config.SDKParamType.HYAI_MOBILE_SDK_PARAM_FACE_DETECT_SMOOTH_THRESHOLD = 5  # 设置人脸106点的阈值[0.0, 1.0], pc端默认0.5，移动端默认值1.0，值越大, 点越稳定, 但相应点会有滞后. 例如0.0是无平滑，0.5轻度平滑，1.0重度平滑。
config.SDKParamType.HYAI_MOBILE_SDK_PARAM_FACE_EXTRA_SMOOTH_THRESHOLD = 6  # 设置人脸134点的阈值[0.0, 1.0], pc端默认0.5，移动端默认值1.0，值越大, 点越稳定, 但相应点会有滞后. 例如0.0是无平滑，0.5轻度平滑，1.0重度平滑。
config.SDKParamType.HYAI_MOBILE_SDK_PARAM_FACE_DETECT_INTERVAL = 7  # 设置tracker每多少帧进行一次detect(默认值有人脸时30,无人脸时每一帧都检测).设置范围是[5, 50] 值越大,cpu占用率越低, 但检测出新人脸的时间越长.
...
# 图片模式的createConfig
config.picCreateConfig = (
    config.HY_MOBILE_DETECT_MODE_IMAGE | config.HY_MOBILE_BACKEND_CPU
)
# 视频模式的createConfig
config.vidCreateConfig = (
    config.HY_MOBILE_DETECT_MODE_VIDEO | config.HY_MOBILE_BACKEND_CPU
)
# GPU模式
