...
def convert_coco_format_left_label(json_head, image_info, annotation_info, hand_type=None, handness=None):
    if handness is not None:
        annotation_info['hand_ness'] = float(handness)
    annotation_info['hand_type'] = hand_type
    json_head['images'].append(image_info)
    json_head['annotations'].append(annotation_info)


def convert_coco_format_v2_6(json_head, image_info, annotation_info, hand_type=None, handness=None):
    if handness is not None:
        annotation_info['hand_ness'] = float(handness)
    annotation_info['hand_type'] = hand_type
    json_head['images'].append(image_info)
