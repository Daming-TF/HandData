"""
    根据打回的json文件，重新覆盖我们的coco-json文件
"""
import json
...
def set_parser():
    ...
    parser.add_argument("--TagJsonPath", help="重标打回数据的json文件路径",
                        default=r"E:\数据标记反馈\hardcase(5479)")
    ...
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"G:\test_data\hardcase_data\hardcase\annotations\{mode}2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"G:\test_data\hardcase_data\hardcase\annotations\{mode}2017-update.json")
    ...
    parser.add_argument("--SearchPath", help="需要知道已用图像id",
                        default=r"G:\test_data\hardcase_data\hardcase\images")
...
def get_keypoints(label_feature):
    ...
    if not np.all(hand1[landmark_index] == 0):
        hand1[:, 2] = 2
...
def main():
    args = set_parser()
    tag_json_path = args.TagJsonPath
    json_dir = args.JsonDir
    data_path = args.DataPath
    json_save_dir = args.JsonSaveDir
    image_save_path = args.ImgSavePath
    ...
    for json_file in json_files:
        ...
        for index in tqdm(range(len(tag_json_date))):
            ...
            image_dir = os.path.join(data_path, file_name, 'images', image_name)

            # 遍历coco-json文件信息，匹配路径，把需要覆盖的图片找出来，并删掉所有图片与坐标信息
            for i in range(len(images_list)):
                images_info = images_list[i]
                original_dir = images_info['original_dir']
            ...
            if update_record != 0:
                ...
                for handlandmarks in handlandmarks_list:
                    mode = 'test2017'
                    flag = convert_coco_format(image_dir, handlandmarks, json_data, mode, image_save_path, coco_id)
                    if flag == 1:
                        coco_id += 1

        # dele_records = list(reversed(serial_number_to_delete))
        serial_number_to_delete.sort()
        dele_records = serial_number_to_delete[::-1]
        for dele_record in dele_records:
            del json_data['images'][dele_record]
            del json_data['annotations'][dele_record]
