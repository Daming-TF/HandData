...
badcase_txt = r'G:\Model_output\Output\HO3D_v3\total_badcase_train.txt'
badcase_image_path = r'G:\imgdate2'
# image_path = r'E:\Data\landmarks\HFB\halpe_Full-Body_Human_Keypoints_and_HOL-Det_dataset\hico_20160224_det\images'
image_path = r'G:\imgdate2\HO3D_v3'

save_path = r'G:\Model_output\Output\HO3D_v3\badcase'
...
def main():
    ...
    for i in range(len(badcase_info)):
        ...
        badcase_num = len(badcase_image_dir_list)

    print("Start to load the img paths to the list")
    img_path_list = []
    file_names_level1 = os.listdir(image_path)
    for file_name_level1 in file_names_level1:
        if not file_name_level1 in ["train"]:
            continue
        # ./HO3D_v3/train
        dir_level1 = os.path.join(image_path, file_name_level1)

        file_names_level2 = os.listdir(dir_level1)
        for file_name_level2 in file_names_level2:
            # ./HO3D_v3/train/ABF10/rgb
            dir_level2 = os.path.join(dir_level1, file_name_level2, 'rgb')

            img_names = os.listdir(dir_level2)
            print(f"loading {file_name_level2}.........")
            for img_name in img_names:
                if img_name.split(".")[-1] == "jpg":
                    # ./evaluation/ABF10/rgb/0000.jpg
                    img_path = os.path.join(dir_level2, img_name)
                    img_path_list.append(img_path)

        print(">> Finished loading all the data <<")
        print("Start to run multi-Progress..........")
        block_len = len(img_path_list) // process_num
        demarcation = 0
        image_blocks = []
        for i in range(process_num):
            if not i == process_num-1:
                image_block = img_path_list[demarcation: demarcation+block_len]
                image_blocks.append(image_block)
                demarcation = demarcation + block_len

            else:
                image_block = img_path_list[demarcation:]
                image_blocks.append(image_block)
        ...
        process_list = [Process(target=run_process,
                                args=(badcase_image_dir_list, image_blocks[i], None, badcase_num, i, mode))
                        for i in range(process_num)]
        i = 0
        while 1:
