...
def set_parser():
    ...
    parser.add_argument("--TagJsonPath", help="重标打回数据的json文件路径",
                        default=r"E:\数据标记反馈\ho3d(6127)\批次样本")
    parser.add_argument("--DataPath", help="匹配覆盖对应路径图片的信息，所需要的原图数据路径",
                        default=r"G:\imgdate2\HO3D_v3\HO3D_v3\train")
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"G:\imgdate2\HO3D_v3\HO3D_from_whole_body\annotations\person_keypoints_{mode}_flip.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"G:\imgdate2\HO3D_v3\HO3D_from_whole_body\annotations\person_keypoints_{mode}-update-for-batchdata.json")
