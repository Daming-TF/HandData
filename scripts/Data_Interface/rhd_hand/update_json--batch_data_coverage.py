...
def set_parser():
    ...
    parser.add_argument("--TagJsonPath", help="重标打回数据的json文件路径",
                        default=r"E:\数据标记反馈\rhd(6219)\批次数据")
    ...
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"F:\image\Rendered Handpose Dataset Dataset\RHD\RHD_from_whole_body\annotations\person_keypoints_{mode}2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"F:\image\Rendered Handpose Dataset Dataset\RHD\RHD_from_whole_body\annotations\person_keypoints_{mode}2017-update-for-batchdata.json")
    parser.add_argument("--ImgSavePath", help="需要找到原图路径作为匹配依据",
                        default=r"F:\image\Rendered Handpose Dataset Dataset\RHD\RHD_from_whole_body")
    parser.add_argument("--SearchPath", help="需要知道已用图像id",
                        default=r"F:\image\Rendered Handpose Dataset Dataset\RHD\RHD_from_whole_body\images")
