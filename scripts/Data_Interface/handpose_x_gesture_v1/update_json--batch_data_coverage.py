...
def set_parser():
    ...
    parser.add_argument("--TagJsonPath", help="重标打回数据的json文件路径",
                        default=r"E:\数据标记反馈\cmu-panopticdb(6135)\批次样本")
    parser.add_argument("--DataPath", help="匹配覆盖对应路径图片的信息，所需要的原图数据路径",
                        default=r"F:\image\CMU\hand143_panopticdb\hand143_panopticdb\imgs")     # 设置这里可以参考check_mark_diff里面的data_path
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"F:\image\CMU\hand143_panopticdb\hand143_pannopticdb_from_whole_body\annotations\person_keypoints_{mode}2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"F:\image\CMU\hand143_panopticdb\hand143_pannopticdb_from_whole_body\annotations\person_keypoints_{mode}2017-update-for-batchdata.json")
    ...
    parser.add_argument("--SearchPath", help="需要知道已用图像id",
                        default=r"F:\image\CMU\hand143_panopticdb\hand143_pannopticdb_from_whole_body\images")
...
def main():
    ...
    for json_file in json_files:
        ...
        for index in tqdm(range(len(tag_json_date))):
            ...
            image_dir = os.path.join(data_path, image_name)

            # 遍历coco-json文件信息，匹配路径，把需要覆盖的图片找出来，并删掉所有图片与坐标信息
            for i in range(len(images_list)):
                images_info = images_list[i]
                original_dir = images_info['image_dir']
                norm_original_dir = os.path.normpath(original_dir)
                norm_image_dir = os.path.normpath(image_dir)
                if norm_original_dir == norm_image_dir:
                    update_record += 1
                    serial_number_to_delete.append(i)

            if update_record == 0:
                print(f'no matched with >> {image_dir}')
            else:
                print(f'update > {update_record} < json units')
                ...
                for handlandmarks in handlandmarks_list:
                    ...
                    mode = 'test2017'
                    flag = convert_coco_format_from_wholebody(image_dir, handlandmarks, json_data, mode, image_save_path, coco_id)
                    if flag == 1:
                        coco_id += 1
