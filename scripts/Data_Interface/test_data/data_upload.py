...
# parser.add_argument("--img_folder", default=fr"E:\test_data\test_data_from_whole_body\images", help="img folder that includes all imgs")
parser.add_argument("--json_folder", default=fr"E:\test_data\test_data_from_whole_body\upload_remark", help="json folder that has corresponding json files")      # G:\test_data\hardcase_data\dataset\{name}\need_to_tag_data\anno
...
def get_label_feature(json_path):
    with open(json_path, 'r') as f:
        ...
        for i, keypoint in enumerate(keypoints):
            preds = np.array(keypoint).reshape(21, 3)
            if np.all(preds == 0):
                continue
...
def main():
    ...
    for json_dir in json_paths:
        with open(json_dir, 'r')as f:
            json_data = json.load(f)
            img_dir = json_data['images'][0]['image_dir']
