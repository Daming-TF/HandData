...
def set_parser():
    ...
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"E:\test_data\test_data_from_whole_body\record\average_pseudo_labels-coco_wo_invalid.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"E:\test_data\test_data_from_whole_body\record\person_keypoints_{mode}-update-for-batchdata.json")
    parser.add_argument("--ImgSavePath", help="需要找到原图路径作为匹配依据",
                        default=r"E:\test_data\test_data_from_whole_body")
        ...
        for index in tqdm(range(len(tag_json_date))):
            ...
            if update_record == 0:
                ...
                for handlandmarks in handlandmarks_list:
                    ...
                    # img_path, landmarks, json_file, img_id
                    flag = convert_coco_format_from_wholebody(image_dir, handlandmarks, json_data, hand_coco_id, image_coco_id, mode, image_save_path)
                    if flag == 1:
                        hand_coco_id += 1
                image_coco_id += 1
