...
def main():
    ...
    with open(json_path, 'r') as whole_json:
        ...
        root = whole_json_data['root']
        image_json_info, annotations_json_info = [], []
        for index in range(len(root)):
            img_dir = os.path.join(image_path, root[index]['img_paths'].strip(""))
            hand_pts = root[index]['joint_self']
            if type(hand_pts) == list:
                image_json_info.append(img_dir)
                annotations_json_info.append(np.array(hand_pts))
        print(f'There is {len(image_json_info)} pic date')
