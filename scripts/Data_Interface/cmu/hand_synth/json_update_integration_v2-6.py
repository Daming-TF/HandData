...
def set_parser():
    ...
    parser.add_argument("--InvalidJsonDir", help="无效数据的json文件路径",
            default=r"E:\数据标记反馈\cmu-synth(6131)\无效样本")
    parser.add_argument("--JsonPath", help="数据集json文件路径——cocoJson文件路径",
            default=rf"F:\image\CMU\hand_labels_synth\hand_labels_synth_from_whole_body_v2_6\annotations\person_keypoints_train2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"F:\image\CMU\hand_labels_synth\hand_labels_synth_from_whole_body_v2_6\annotations")       # person_keypoints_{mode}-update.json
