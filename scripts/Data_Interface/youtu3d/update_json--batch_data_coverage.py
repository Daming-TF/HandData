...
def set_parser():
    ...
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"E:\Data\landmarks\YouTube3D\YouTube3D-wholebody\annotations\{mode}2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"E:\Data\landmarks\YouTube3D\YouTube3D-wholebody\annotations\{mode}2017-update-for-batchdata.json")
    parser.add_argument("--ImgSavePath", help="图片保存所在的路径",
                        default=r"E:\Data\landmarks\YouTube3D\YouTube3D-wholebody")
    parser.add_argument("--SearchPath", help="需要知道已用图像id",
                        default=r"E:\Data\landmarks\YouTube3D\YouTube3D-wholebody\images")
...
def main():
    ...
    image_id = start_id + COCO_START_ID
    coco_id = len(json_data['images'])
        ...
        for index in tqdm(range(len(tag_json_date))):
            ...
            if update_record == 0:
                ...
                for handlandmarks in handlandmarks_list:
                    ...
                    mode = 'test'
                    # convert_coco_format_from_wholebody(img_id, image_dir, landmarks, json_file, coco_id,
                    #                         save_path, mode, coco_factor=COCOBBOX_FACTOR):
                    flag = convert_coco_format_from_wholebody(image_id, image_dir, handlandmarks, json_data, coco_id, image_save_path, mode)
                    if flag == 1:
                        coco_id += 1
                if flag == 1:
                    image_id += 1
