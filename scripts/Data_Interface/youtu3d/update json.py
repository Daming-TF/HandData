...
def set_parser():
    ...
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"E:\Data\landmarks\YouTube3D\YouTube3D-crop\annotations\{mode}2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"E:\Data\landmarks\YouTube3D\YouTube3D-crop\annotations\{mode}2017-update.json")
    parser.add_argument("--ImgSavePath", help="需要找到原图路径作为匹配依据",
                        default=r"E:\Data\landmarks\YouTube3D\YouTube3D-crop")
...
def main():
    ...
    for json_file in json_files:
        ...
        # 遍历打回数据的信息，匹配原图路径
        for index in range(len(tag_json_date)):
            update_record = 0
