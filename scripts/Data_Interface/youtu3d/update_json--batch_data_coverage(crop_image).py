...
def set_parser():
    ...
    parser.add_argument("--TagJsonPath", help="重标打回数据的json文件路径",
                        default=r"E:\数据标记反馈\youtu3d\打回数据")
    parser.add_argument("--DataPath", help="匹配覆盖对应路径图片的信息，所需要的原图数据路径",
                        default=r"E:\Data\landmarks\YouTube3D\images")
    parser.add_argument("--JsonDir", help="数据集json文件路径——cocoJson文件路径",
                        default=rf"E:\Data\landmarks\YouTube3D\YouTube3D-crop2\annotations\{mode}2017.json")
    parser.add_argument("--JsonSaveDir", help="更新后文件写入路径",
                        default=rf"E:\Data\landmarks\YouTube3D\YouTube3D-crop2\annotations\{mode}2017-update-for-batchdata.json")
    parser.add_argument("--ImgSavePath", help="图片保存所在的路径",
                        default=r"E:\Data\landmarks\YouTube3D\YouTube3D-crop2")
    parser.add_argument("--SearchPath", help="需要知道已用图像id",
                        default=r"E:\Data\landmarks\YouTube3D\YouTube3D-crop2\images")
...
def main():
    ...
    for json_file in json_files:
        ...
        for index in tqdm(range(len(tag_json_date))):
            ...
            data_name = image_name_list[0]
            image_dir = os.path.join(data_path, data_name+'2017', image_name)
            ...
            for i in (range(len(images_list))):
                ...
                if original_dir == image_dir:
                    if original_dir == r'E:\Data\landmarks\YouTube3D\images\test2017\00012.jpg':
                        print()
                    update_record += 1
            ...
            if update_record == 0:
                ...
                for handlandmarks in handlandmarks_list:
                    ...
                    flag = convert_coco_format_from_crop(file_name, image_dir, handlandmarks, json_data,
                                                         mode, image_save_path, coco_id)
                    # flag = convert_coco_format_from_crop(image_dir, handlandmarks, json_data, mode, image_save_path, coco_id)
                    if flag == 1:
                        coco_id += 1
